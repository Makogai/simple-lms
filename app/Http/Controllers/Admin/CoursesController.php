...
class CoursesController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->isProfessor) {
            $courses = Course::where('teacher_id', auth()->user()->id)->with(['teacher', 'students', 'media'])->get();
        }
    ...
    public function create()
    {
        ...
        if (auth()->user()->isProfessor){
            $teachers = User::where("id", Auth::id())->pluck('name', 'id')->prepend(trans('global.pleaseSelect'), '');
        }

        $students = User::all();
        $students = $students->reject(function ($user, $key) {
